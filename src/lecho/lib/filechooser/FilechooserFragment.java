...
public class FilechooserFragment extends Fragment implements
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View rootView = inflater.inflate(R.layout.fragment_filechooser,
				container, false);

		currentDirView = (TextView) rootView.findViewById(R.id.fc_path);

		viewSwitcher = (ViewSwitcher) rootView
				.findViewById(R.id.fc_view_switcher);
	...
	public void onActivityCreated(@Nullable Bundle savedInstanceState) {
		...
		SelectionMode tempSelectionMode = (SelectionMode) getActivity()
		...
		ItemType tempItemType = (ItemType) getActivity().getIntent()
				.getSerializableExtra(FilechooserActivity.BUNDLE_ITEM_TYPE);
		...
		if (null == savedInstanceState) {
			...
			String currentDirPath = savedInstanceState
					.getString(BUNDLE_CURRENT_DIR);
		...
		// Set up adapter
		adapter = new PathAdapter(getActivity(), itemClickListener,
				itemLongClickListener, checkListener, checkboxValidator);
	...
	public Loader<List<File>> onCreateLoader(int id, Bundle data) {
		if (LOADER_ID == id) {
			...
			return new PathLoader(getActivity(), currentDir, fileFilter,
					shouldParseMounts);
		}
		return null;
	}
	...
	private class ConfirmButtonClickListener implements View.OnClickListener {
		...
		public void onClick(View v) {
			...
			ArrayList<String> names = new ArrayList<String>(pathsSize);
			File file = null;
			for (int i = 0; i < pathsSize; ++i) {
				int position = adapter.getCheckedPositions().keyAt(i);
				paths.add(((File) adapter.getItem(position)).getAbsolutePath());
				names.add(((File) adapter.getItem(position)).getName());
				file = (File) adapter.getItem(position);
			}

			Intent data = new Intent();
			data.putStringArrayListExtra(
					FilechooserActivity.BUNDLE_SELECTED_PATHS, paths);
			data.putStringArrayListExtra(
					FilechooserActivity.BUNDLE_SELECTED_NAMES, names);
			data.putExtra(FilechooserActivity.BUNDLE_SELECTED_FILES, file);
			getActivity().setResult(Activity.RESULT_OK, data);
			getActivity().finish();
		}
	}

	private class DefaultListItemClickListener implements
			OnFcListItemClickListener {
		...
		protected void onFileClick(int position, File file) {
			if (ItemType.FILE.equals(itemType) || ItemType.ALL.equals(itemType)) {
				...
				boolean isChecked = !adapter.getCheckedPositions()
						.get(position);
				adapter.checkPosition(position, isChecked);

				if (isChecked
						&& SelectionMode.SINGLE_ITEM.equals(selectionMode)) {
	...
	private class FileOnlyCheckboxValidator implements
			PathAdapter.FcCheckboxValidator {
	...
	private class DirectoryOnlyCheckboxValidator implements
			PathAdapter.FcCheckboxValidator {
	...
	private class AllItemsVisibleCheckboxValidator implements
			PathAdapter.FcCheckboxValidator {
	...
	private class DefaultItemLongClickListener implements
			OnFcListItemLongClickListener {

		@Override
		public boolean onItemLongClick(int position, File file) {
			FileDetailsDialogFragment.showDialog(getActivity(),
