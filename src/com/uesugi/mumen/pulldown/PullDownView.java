package com.uesugi.mumen.pulldown;
...
import com.uesugi.mumen.pulldown.ScrollOverListView.OnScrollOverListener;
...
public class PullDownView extends LinearLayout implements OnScrollOverListener {
	...
	private static final int START_LOAD = 6; // 开始Load
	public static int DEFAULT_HEADER_VIEW_HEIGHT = Constants.width5_1; // 头部文件原本的高度
	...
	public void startLoad() {
		mUIHandler.sendEmptyMessage(START_LOAD);
	}
	...
	private void checkHeaderViewState() {
		if (mHeaderViewParams.height >= Constants.width5_1) {
	...
	class ShowHeaderViewTask extends TimerTask {
		...
		public void run() {
			...
			mHeaderIncremental -= AUTO_INCREMENTAL;
			if (mHeaderIncremental > Constants.width5_1) {
				mUIHandler.sendEmptyMessage(WHAT_SET_HEADER_HEIGHT);
			} else {
				mHeaderIncremental = Constants.width5_1;
				mUIHandler.sendEmptyMessage(WHAT_SET_HEADER_HEIGHT);
				if (!mIsRefreshing) {
					mIsRefreshing = true;
	...
	private Handler mUIHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case START_LOAD: {
				setHeaderHeight(DisplayUtil.dip2px(getContext(), 100));
	...
	public boolean onMotionDown(MotionEvent ev) {
		mIsDown = true;
		mIsPullUpDone = false;
		mMotionDownLastY = ev.getRawY() + 50;
		return false;
	}
	...
	public boolean onMotionUp(MotionEvent ev) {
		...
		if (mHeaderViewParams.height > 0) {
			...
			int x = mHeaderIncremental - Constants.width5_1;
